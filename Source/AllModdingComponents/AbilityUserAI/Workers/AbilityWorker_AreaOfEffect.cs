...
{
    /// <summary>
    /// Helps calculating the final Ability power and targeting location which take in account the density of targeted pawns. Is friendly to being extended.
    /// </summary>
    public class AbilityWorker_AreaOfEffect : AbilityWorker
    {
        ...
        {
            float baseScore = abilityDef.power;
            ...
            foreach(Pawn targetPawn in AbilityUtility.GetPawnsInsideRadius(bestTarget, pawn.Map, abilityDef.abilityRadius, 
                predPawn => (abilityDef.abilityRadiusNeedSight && GenSight.LineOfSight(pawn.Position, predPawn.Position, pawn.Map, true)) || (abilityDef.abilityRadiusNeedSight == false)))
            {
                if(targetPawn.HostileTo(pawn) || targetPawn.AnimalOrWildMan())  //Hostile pawns or animals increase score.
