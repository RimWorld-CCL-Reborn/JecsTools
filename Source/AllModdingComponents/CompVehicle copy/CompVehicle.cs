    ...
    public class CompVehicle : ThingComp
    {
                            ...
                            for (int i = 0; i < group.handlers.Count; i++)
                            {
                                ...
                                for (int j = 0; j < pawnNeeds.Count; j++)
                                {
                                    if (pawnNeeds[j].CurLevelPercentage < this.Props.ejectIfBelowNeedPercent)
                                    {
                                        ...
                                        //Removed part of Logical Condition since apparently pawns that are forming caravans are not part of a caravan?
                                        //Added conditonals so that pawns won't ditch the vehicle when you're at the edge of the map and will wait until after the vehicle has unloaded before leaving
                                        if (!((this.Pawn.GetLord()?.LordJob?.ToString()) == "RimWorld.LordJob_FormAndSendCaravan") && (!((this.Pawn.CurJob?.def) == JobDefOf.UnloadYourInventory)&& !this.Pawn.Position.InNoBuildEdgeArea(this.Pawn.Map)))
                                        {
                                            ...
                                            //Addition to make sure that if the pawn is related in caravan 
                                            if (!this.Pawn.IsFighting() && !pawn.IsFighting())
                                            {
                                                //Notify the player that "Johnny" has left the vehicle so the pawn can be punished as appropriate&& !this.Pawn.Map.mapPawns.AllPawns.Contains(pawn)
                                                if (!this.Pawn.Map.mapPawns.AllPawns.Contains(pawn))
                                                {
                                                    ...
                                                    //Log.Error(group.handlers.Any.ToString());
                                                    break;
                                                }
                                                else
                                                {
                                                    //Fixes pawn cloning that results when pawns are ejected due to needs after entering the map
                                                    //Don't know why group.handlers.Remove() wasn't working, although I was told that it could be immutable in the modding channel
                                                    ThingOwner<Pawn> handler = new ThingOwner<Pawn>();
                                                    for (int z = 0; z < group.handlers.Count;z++){
                                                        if (group.handlers[z].def != pawn.def){
                                                            handler.TryAdd(pawn,1,true);
                                                        }
                                                    }
        ...
        public void ResolveStatus()
        {
            ...
            if (this.movingStatus == MovingState.able)
            {
                ...
                //Apparently pawns forming a caravan are not considered part of a caravan
